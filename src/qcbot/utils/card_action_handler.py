"""
Card Action Handler Utility for QC Bot

This utility handles adaptive card actions and submissions,
...
class CardActionHandler:
    ...
    def __init__(self, feedback_service: MemoryEfficientFeedbackService, teams_adapter: TeamsAdapter):
    ...
    async def handle_dismiss_feedback(
        ...
        try:
            await self.teams_adapter.send_message(
                service_url, conv_id,
                "No problem! Feel free to reach out anytime you need QC assistance."
            )
    ...
    async def handle_submit_feedback(
        ...
        try:
            ...
            # Get existing card activity ID and remove from tracking
    ...
    async def handle_message_reply_feedback(
        ...
        """Handle message reply feedback action."""
        try:
            ...
            if message_id and standardized_feedback:
                ...
                logger.warning(f"Incomplete message reply feedback data: message_id={message_id}, feedback={standardized_feedback}")
    ...
    async def handle_show_resources(
        self, 
        conv_id: str, 
        service_url: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # Create and send the resources card
            resources_card = create_resources_card()
            adapter = TeamsAdapter()
            await adapter.send_card(service_url, conv_id, resources_card)
            
            logger.info(f"Successfully sent resources card to conversation {conv_id}")
            
        except Exception as e:
            logger.error(f"Error sending resources card: {e}")
            # Send a fallback message if the card fails
            try:
                from qcbot.infrastructure.teams_adapter import TeamsAdapter
                adapter = TeamsAdapter()
                await adapter.send_message(
                    service_url, conv_id,
                    "Sorry, I couldn't load the resources right now. Please try again later."
                )
