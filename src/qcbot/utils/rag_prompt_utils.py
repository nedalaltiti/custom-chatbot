...
class RAGPromptUtils:
    ...
    def format_chunks_for_prompt(chunks: List["RetrievedChunk"]) -> str:
        """Format chunks for comprehensive LLM prompt with enhanced context and prioritization."""
        if not chunks:
        ...
        # Enhanced selection for more comprehensive answers
        if high_conf:
            ...
            # Increased selection for more comprehensive answers
            selected_chunks = (high_conf_sorted[:15] + med_conf_sorted[:8] + low_conf_sorted[:3])[:18]
        else:
            # Standard selection if no high-confidence chunks
            med_conf_sorted = sorted(med_conf, key=lambda x: (x.relevance_score, hash(x.content)), reverse=True)
            low_conf_sorted = sorted(low_conf, key=lambda x: (x.relevance_score, hash(x.content)), reverse=True)
            selected_chunks = (med_conf_sorted[:12] + low_conf_sorted[:5])[:15]
        ...
        for i, chunk in enumerate(selected_chunks):
            ...
            confidence_level = "High" if chunk.relevance_score >= 0.7 else "Medium" if chunk.relevance_score >= 0.4 else "Low"
            
            # Add document type and confidence for better context
            header = f"[Document: {source}{file_type}, Section: {chunk_num}, Confidence: {confidence_level}]"
            formatted_chunks.append(f"{header}\n{chunk.content}")
        
        # Add summary of information sources
        sources_summary = f"\n\n[Information Sources: {len(selected_chunks)} relevant sections from {len(set(c.metadata.get('source', 'Unknown') for c in selected_chunks))} documents]"
