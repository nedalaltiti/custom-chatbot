"""
Configuration settings for QC Bot.

This module provides simplified settings for the QC chatbot.
"""

import logging
from dataclasses import dataclass, field
from typing import Optional, List
from qcbot.config.environment import (
    get_env_var, get_env_var_bool, get_env_var_float, get_env_var_int, get_env_var_list
)

logger = logging.getLogger("qcbot.config")
...
class DatabaseSettings:
    ...
    def from_environment(cls) -> "DatabaseSettings":
        ...
        if use_aws_secrets and not skip_db_init:
            try:
                from qcbot.utils.secret_manager import get_database_credentials, get_aws_region
...
class GeminiSettings:
    ...
    max_output_tokens: int = 2048  # Reduced from 4096 to 2048 (half the previous value)
    ...
    def from_environment(cls) -> "GeminiSettings":
        ...
        if use_aws_secrets_global and secret_name:
            try:
                from qcbot.utils.secret_manager import load_gemini_credentials, get_aws_region
...
class TeamsSettings:
    ...
    def from_environment(cls) -> "TeamsSettings":
        ...
        try:
            ...
            logger.info(f"Loading Teams settings for QC bot: {app_config.name}")
        except Exception as e:
...
class FeedbackSettings:
    ...
    feedback_timeout_minutes: int = 10  # Restored default to 10 minutes

    @classmethod
    def from_environment(cls) -> "FeedbackSettings":
        return cls(
            admin_token=get_env_var("FEEDBACK_ADMIN_TOKEN", cls.admin_token),
            feedback_timeout_minutes=get_env_var_float("FEEDBACK_TIMEOUT_MINUTES", cls.feedback_timeout_minutes),
        )
...
class PerformanceSettings:
    ...
    use_intent_classification: bool = True  # Enable intent classification for QC bot
    cache_embeddings: bool = True
    cache_ttl_seconds: int = 3600
    min_streaming_length: int = 200
    show_acknowledgment_threshold: int = 10
    enable_streaming: bool = True
    streaming_delay: float = 0.8
    max_chunk_size: int = 120
    
    # Document processing settings for QC
    chunk_size: int = 1500
...
class AppSettings:
    ...
    def from_environment(cls) -> "AppSettings":
        ...
        return cls(
            ...
            app_name="QC Bot",  # Always use QC Bot for this application
