...
from qcbot.utils.result import Result, Success, Error
from qcbot.utils.error import LLMError, ErrorCode
from qcbot.config.settings import settings
from qcbot.config.environment import get_env_var_bool
...
class GeminiService:
    ...
    async def analyze_messages_streaming(self, messages: List[str]) -> AsyncGenerator[str, None]:
        ...
        # Check if the last message (current query) is empty
        if not messages[-1] or not messages[-1].strip():
            yield "I need a question to answer."
            return
