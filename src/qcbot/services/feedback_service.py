...
class FeedbackService:
                ...
                if inactive_duration >= target_inactivity:
                    ...
                    if user_id not in self.feedback_sent:
                        ...
                        if activity_id:
                            ...
                            if on_card_sent:
                                logger.info(f"Calling on_card_sent callback with conversation_id={conversation_id}, activity_id={activity_id}")
                                on_card_sent(conversation_id, activity_id)
                            else:
                                logger.warning(f"No on_card_sent callback provided for user {user_id}")
                        else:
    ...
    async def send_feedback_prompt(self, service_url: str, conversation_id: str):
        ...
        try:
            ...
            if activity_id:
                logger.info(f"Successfully sent feedback card to conversation {conversation_id} with activity ID: {activity_id}")
            else:
                logger.warning(f"Failed to send feedback card to conversation {conversation_id} - no activity ID returned")
    ...
    async def find_recent_bot_message_id(self, conversation_id: str) -> int | None:
        """Return the most recent bot message DB ID for a conversation, or None.

        Used as a fallback when Teams reply_to_id cannot be mapped to our DB.
        """
        try:
            async with get_db_session_context() as session:
                ...
                msg_id = res.scalar()
                if msg_id:
                    logger.debug(f"Fallback found recent bot message ID {msg_id} for conversation {conversation_id}")
                return msg_id
        except Exception as e:
            logger.warning(f"Failed to find recent bot message for conversation {conversation_id}: {e}")
            return None
            ...
            async with get_db_session_context() as session:
                ...
                if bot_message_db_id:
                    ...
                    try:
                        ...
                        logger.warning(f"Unable to convert message_id '{message_id}' to integer; skipping message_reply_feedback write")
                        return None
                    logger.debug(f"Using message_id {message_id} as bot message DB ID (no mapping found)")
                
                # First, verify that the message exists in the message table
                from qcbot.db.models import Message
                message_exists = await session.get(Message, actual_message_id)
                if not message_exists:
                    logger.warning(f"Message {actual_message_id} does not exist in database - cannot record feedback")
                    return None
                
                # Create a message reply record for this feedback
                # This is needed because MessageReplyFeedback references message_reply.id
                message_reply = MessageReply(
                    message_id=actual_message_id,
                    reply_message_id=actual_message_id,  # Self-reference for feedback
                )
    ...
    async def record_builtin_teams_feedback(self, message_id: int, feedback: str = "", feedback_comment: str = "", user_id: str = None) -> MessageReplyFeedback | None:
        ...
        try:
            async with get_db_session_context() as session:
                # First, verify that the message exists in the message table
                from qcbot.db.models import Message
                message_exists = await session.get(Message, message_id)
                if not message_exists:
                    logger.warning(f"Message {message_id} does not exist in database - cannot record feedback")
                    return None
