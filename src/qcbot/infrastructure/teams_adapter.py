...
class TeamsAdapter:
    ...
    async def send_message_with_resources_button(self, svc_url: str, conv_id: str, text: str) -> Optional[str]:
        """Send a message with both built-in feedback buttons and a resources link."""
        # Add the resources link text before "Is there anything else I can help you with?"
        if "Is there anything else I can help you with?" in text:
            # Replace the ending with resources link + original ending
            resources_text = text.replace(
                "Is there anything else I can help you with?",
                "For further information, click [📋 Resources](show_resources) to access all QC links and forms.\n\nIs there anything else I can help you with?"
            )
        else:
            # Add resources link at the end
            resources_text = f"{text}\n\nFor further information, click [📋 Resources](show_resources) to access all QC links and forms."
        
        # Send the message with built-in feedback
        payload = {
            "type": "message", 
            "text": resources_text, 
            "textFormat": "markdown",
        ...
        ok, activity_id = await self._post_activity(svc_url, conv_id, payload, return_id=True)
        if ok:
            logger.info(f"Sent message with resources link: activity_id={activity_id}")
        else:
            logger.warning("Failed to send message with resources link")
        return activity_id if ok else None

    async def send_card(self, svc_url: str, conv_id: str, card: dict) -> Optional[str]:
        logger.info(f"Sending card to conversation {conv_id}")
        payload = {
            "type": "message",
            "attachments": [{
        ...
        ok, activity_id = await self._post_activity(svc_url, conv_id, payload, return_id=True)
        if ok and activity_id:
            logger.info(f"Successfully sent card to conversation {conv_id} with activity ID: {activity_id}")
        else:
            logger.warning(f"Failed to send card to conversation {conv_id} - ok={ok}, activity_id={activity_id}")
        return activity_id if ok else None

    async def update_card(self, svc_url: str, conv_id: str, act_id: str, card: dict) -> bool:
        logger.info(f"🔄 Updating card for conversation {conv_id} with activity ID {act_id}")
        ...
        resp = await client.post(self.BOT_TOKEN_URL, data=payload)
        if resp.is_success:
            logger.info(f"✅ Successfully updated card for conversation {conv_id}")
            return True
    ...
    async def delete_activity(self, svc_url: str, conv_id: str, act_id: str) -> bool:
        ...
        logger.info(f"🗑️ Attempting to delete activity {act_id} from conversation {conv_id}")
        token = await self.get_bot_token()
        headers = {
        ...
        url = f"{svc_url.rstrip('/')}/v3/conversations/{conv_id}/activities/{act_id}"
        logger.info(f"🗑️ DELETE URL: {url}")
        client = await _get_http()
        resp = await client.delete(url, headers=headers)
        if resp.is_success:
            logger.info(f"✅ Successfully deleted activity {act_id}")
            return True
...
class _MicrosoftTeamsStreamer:
    ...
    async def _finish(self) -> bool:
        ...
        # Add resources link text to the message before sending
        if self.finished and self.buffer:
            # Add the resources link text before "Is there anything else I can help you with?"
            if "Is there anything else I can help you with?" in self.buffer:
                # Replace the ending with resources link + original ending
            ...
            # Update the body with the modified text
            body["text"] = self.buffer
