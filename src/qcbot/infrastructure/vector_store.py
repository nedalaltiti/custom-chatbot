"""
In-house NumPy/ndarray vector store (disk-backed, no FAISS) for QC bot.
"""
...
from qcbot.infrastructure.embeddings import VertexDirectEmbeddings
...
class VectorStore:
    """Minimal but solid disk-backed cosine-similarity store with caching for QC bot."""
    ...
    def __init__(
        self,
        *,
        collection_name: str = "qc_documents",
    ...
    async def similarity_search(
        ...
        # Validate matrix and documents are in sync
        if self._matrix.shape[0] != len(self._docs):
            logger.error(f"Matrix-docs mismatch! Matrix rows: {self._matrix.shape[0]}, Docs count: {len(self._docs)}")
            return []
        ...
        # Ensure indices are within bounds and filter out invalid ones
        valid_indices = top_idx[top_idx < len(self._docs)]
        if len(valid_indices) == 0:
            logger.warning(f"No valid indices found! Matrix shape: {self._matrix.shape}, Docs count: {len(self._docs)}")
            return []
            
        results = [self._docs[i] for i in valid_indices]
        
        # Debug logging
        logger.debug(f"Similarity scores: {[sims[i] for i in valid_indices[:5]]}")  # Top 5 scores
    ...
    async def close(self):
        ...
        if self._matrix is not None:
            self.sync_disk()
        logger.debug("VectorStore resources cleaned up")
    ...
    class ChromaVectorStore:
        ...
        async def close(self):
            ...
            try:
                ...
                self._client.persist()
                logger.debug("ChromaVectorStore resources cleaned up")
            except Exception as e:
                logger.warning(f"Error during ChromaVectorStore cleanup: {e}")
