...
class RAG:
    ...
    def __init__(
        ...
        *,
        ...
        top_k: Optional[int] = None,  # Will use settings.max_chunks_per_query if not provided
    ) -> None:
        self.vector_store = vector_store or VectorStore()
        self.llm_provider = llm_provider
        self.default_top_k = top_k or settings.performance.max_chunks_per_query
        ...
        logger.info("RAG engine initialized with consistency optimizations (top_k=%s)", self.default_top_k)
    ...
    async def query(
        ...
        *,
        ...
        try:
            ...
            payload["comprehensiveness_score"] = self._assess_comprehensiveness(ranked_chunks)
            payload["total_sources_considered"] = len(ranked_chunks)
            if user_id:
    ...
    def _assess_comprehensiveness(self, chunks: List["RetrievedChunk"]) -> str:
        """Assess how comprehensive the retrieved information is."""
        if not chunks:
            return "Low"
        ...
        high_conf_count = sum(1 for chunk in chunks if chunk.relevance_score >= 0.7)
        
        # Calculate average relevance
        avg_relevance = sum(chunk.relevance_score for chunk in chunks) / len(chunks)
        
        # Assess comprehensiveness based on multiple factors
        if unique_sources >= 3 and high_conf_count >= 3 and avg_relevance >= 0.6:
            return "High"
        elif unique_sources >= 2 and high_conf_count >= 2 and avg_relevance >= 0.4:
            return "Medium"
        else:
            return "Low"
