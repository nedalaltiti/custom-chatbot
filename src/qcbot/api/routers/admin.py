"""
Admin router for managing the QC Teams Bot.
...
from qcbot.core.chunking import save_uploaded_file, reload_knowledge_base, process_document
from qcbot.utils.di import get_vector_store
from qcbot.infrastructure.vector_store import VectorStore
from qcbot.infrastructure.ingest import force_rebuild_embeddings
...
async def reload_kb(store: VectorStore = Depends(get_vector_store)):
    ...
    """
    try:
        file_count = await force_rebuild_embeddings(store)
        return {
            "status": "reloaded",
...
@router.post("/knowledge/force-rebuild")
async def force_rebuild_kb(store: VectorStore = Depends(get_vector_store)):
    """
    ...
    try:
        ...
        return {
            "status": "rebuilt",
            "message": "Embeddings force rebuilt from scratch",
            "files_processed": file_count
        }
    except Exception as e:
        logger.error(f"Error force rebuilding embeddings: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error force rebuilding embeddings: {str(e)}")
