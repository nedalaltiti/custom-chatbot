...
from qcbot.api.routers import admin, feedback, health, teams, debug
from qcbot.config.settings import settings
from qcbot.utils.di import get_vector_store         
from qcbot.infrastructure.ingest import refresh_vector_index 
from qcbot.utils.error import BaseError, ErrorSeverity
from qcbot.services.session_tracker import SessionTracker   
...
logger = logging.getLogger("qcbot.app")
...
async def lifespan(_: FastAPI) -> AsyncGenerator[None, None]:
    ...
    logger.info("QC Bot starting up…")

    # Initialize database connections first
    try:
        from qcbot.db.session import init_database
        await init_database()
    ...
    try:
        ...
        # Get QC app configuration
        app_config = get_current_app_config()
        logger.info(f"QC Bot Configuration:")
        logger.info(f"   • Bot name: {app_config.name}")
        logger.info(f"   • Instance ID: {app_config.instance_id}")
    ...
    try:
        ...
        if _temp_credentials_path:
            try:
                from qcbot.utils.secret_manager import cleanup_temp_credentials
        ...
        try:
            from qcbot.db.session import close_database
...
async def qcbot_error_handler(_: Request, exc: BaseError) -> JSONResponse:
